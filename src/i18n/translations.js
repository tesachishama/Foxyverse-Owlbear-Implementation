/**
 * Single file for all UI translations. Switch locale with setLocale('en'|'fr').
 * Add new keys in English here, then duplicate the object for French.
 */
export const translations = {
  en: {
    appTitle: "Foxyverse TTRPG",
    appSubtitle: "A Foxyverse integration for Owlbear.Rodeo",

    // Sheet selector & permissions
    selectSheet: "Select character sheet",
    newSheet: "New sheet",
    noSheet: "No sheet selected",
    noAvailableSheet: "No available sheet",
    settings: "Settings",
    permissions: "Permissions",
    view: "View",
    edit: "Edit",
    linkToken: "Link to token",
    gmOnly: "GM only",

    // Tabs
    tabBio: "Bio",
    tabStats: "Statistics",
    tabSpells: "Spells",
    tabInventory: "Inventory",
    tabChat: "Chat",
    tabNotes: "Notes",
    tabSettings: "Settings",

    // Bio
    name: "Name",
    surname: "Surname",
    element: "Element",
    class: "Class",
    level: "Level",

    // Stats
    health: "Health",
    mana: "Mana",
    favor: "Favor",
    labelMaximum: "Maximum",
    labelCurrent: "Current",
    labelTemporary: "Temporary",
    statBonusShort: "Bonus",
    defensesBlock: "Defences",
    labelPhysicalShort: "Physical",
    labelMagicalShort: "Magical",
    defenseInfiniteDisplay: "∞",
    graphTitle: "Graph",
    talentsBlock: "Talents",
    talentDefault: "Talent",
    talentDescPlaceholder: "write talent's description here",
    talentDescTooltip: "Talent's description",
    itemTalent: "Item talent",
    none: "—",
    totalPointsLabel: "Total Points",
    ptsSuffix: " pts",
    tempHP: "Temp HP",
    currentHP: "Current HP",
    maxHP: "Max HP",
    currentMP: "Current MP",
    maxMP: "Max MP",
    currentFavor: "Current Favor",
    maxFavor: "Max Favor",
    action: "Action",
    actionModifier: "Action modifier",
    speed: "Speed",
    speedModifier: "Speed modifier",
    baseStat: "Base",
    xpBonus: "XP Bonus",
    itemBonus: "Item",
    passiveBonus: "Passive",
    total: "Total",
    roll: "Roll",
    modifier: "Modifier",
    quickMods: "Quick modifiers",
    knowledge: "Knowledge",
    addKnowledge: "Add knowledge",
    tier: "Tier",
    tier1: "Tier 1 (-1)",
    tier2: "Tier 2 (-3)",
    tier3: "Tier 3 (-5)",
    tier4: "Tier 4 (-10)",
    constitution: "Constitution",
    strength: "Strength",
    intelligence: "Intelligence",
    perception: "Perception",
    social: "Social",
    agility: "Agility",
    focus: "Focus",

    /** Short stat keys for inline roll buttons [str +2] */
    statAbbr_constitution: "con",
    statAbbr_strength: "str",
    statAbbr_intelligence: "int",
    statAbbr_perception: "per",
    statAbbr_social: "soc",
    statAbbr_agility: "agi",
    statAbbr_focus: "foc",

    /** Inline roll type labels (locale-specific command aliases) */
    inlineRoll_pdmg: "pdmg",
    inlineRoll_mdmg: "mdmg",
    inlineRoll_tdmg: "tdmg",
    inlineRoll_heal: "heal",
    inlineRoll_theal: "theal",
    inlineRoll_mana: "mana",
    inlineRoll_roll: "roll",

    // Rolls
    statRoll: "Stat roll",
    nat1: "Nat 1",
    nat20: "Nat 20",
    success: "Success",
    failure: "Failure",
    dc: "DC",
    result: "Result",
    rollModalRerollTitle: "{ordinal} reroll result",
    reroll: "Reroll",
    rerollCost: "Uses 1 Favor",
    /** Roll formula suffix `!<expr`: result cannot be below expr (shown when applied). */
    rollClampFloorNote: "raised to minimum",
    /** Roll formula suffix `!>expr`: result cannot be above expr (shown when applied). */
    rollClampCeilNote: "capped at maximum",
    rollSpeed: "Roll Speed",
    linkTokenToSheet: "Link selected token",
    linkedTokens: "Linked tokens",
    unlink: "Unlink",
    noTokenSelected: "Select a token on the map first",
    physicalDamage: "Physical damage",
    magicDamage: "Magical damage",
    trueDamage: "True damage",
    heal: "Heal",
    overHeal: "Temporary heal (Temp HP)",
    otherRoll: "Other roll",
    applyDamage: "Apply damage",
    applyPhysicalDamages: "Apply physical damages",
    applyMagicalDamages: "Apply magical damages",
    applyTrueDamages: "Apply true damages",
    applyHeal: "Apply heal",
    applyOverHeal: "Apply temp HP",
    healedFor: "healed for",
    tempHealedFor: "temporary healed for",
    tookDamage: "took",
    regainedMana: "regained",
    manaPoints: "mana points",
    chatApplyPhysicalDamage: "{{name}} took {{actual}} physical damages [{{raw}} - {{def}} PD]",
    chatApplyPhysicalDamageElemental: "{{name}} blocked physical damage (elemental) [rolled {{raw}}]",
    chatApplyMagicalDamage: "{{name}} took {{actual}} magical damages [{{raw}} - {{def}} MD]",
    chatApplyMagicalDamageToMana: "{{name}} lost {{actual}} mana from magical damage [2×{{raw}} − {{def}} MD]",
    chatApplyMagicalDamageElemental: "{{name}} took {{total}} magical damage (elemental) [2×{{raw}} − {{def}} MD]; mana lost: {{manaLost}} (after temp HP).",
    chatApplyTrueDamage: "{{name}} took {{actual}} true damages",
    chatApplyTrueDamageToMana: "{{name}} lost {{actual}} mana from true damage",
    chatApplyTrueDamageElemental: "{{name}} took {{total}} true damage (elemental); mana lost: {{manaLost}} (after temp HP).",
    chatCurrencyGift: "{{fromName}} {{gave}} {{gold}} {{goldLabel}}, {{silver}} {{silverLabel}}, {{copper}} {{copperLabel}} {{to}} {{toName}}",
    chatItemGift: "{{fromName}} {{gave}} {{qty}} × {{itemName}} {{to}} {{toName}}",
    gainedVerb: "gained",
    temporaryHitPointsPhrase: "temporary hit points",
    alreadyAtFullHealth: "was already at full health",
    nowAtFullHealth: "is now at full health",
    alreadyAtFullMana: "was already at full mana",
    nowAtFullMana: "is now at full mana",

    // Spells
    spellName: "Spell name",
    spellEffect: "Effect",
    spellCost: "Cost",
    costHP: "HP",
    costMP: "MP",
    use: "USE",
    used: "Used",
    times: "times",
    continuous: "Continuous",
    spellArmed: "Armed",
    cost: "cost",
    toggle: "Toggle",
    reorder: "Reorder",
    selectSpellToRemove: "Select a spell to remove",
    deductCost: "Deduct cost",
    notEnoughMP: "Not enough MP — use HP?",
    confirmUseHP: "Using HP for MP will drain life. Continue?",
    confirmCastBelowZeroHP: "Casting this spell would leave you at 0 HP or below. Are you sure you want to do that?",

    // Stats (template UI)
    talentName: "Talent name",
    detailedStatistics: "Detailed Statistics",
    overwriteBonusesHere: "Overwrite bonuses here",
    points: "Points",
    save: "Save",
    confirmDelete: "Are you sure you want to delete?",
    confirmModalTitle: "Please confirm",
    confirmDeleteSheetBody: "Delete the sheet \"{{name}}\"? This cannot be undone.",
    confirmImportOverwriteBody: "Replace \"{{name}}\" with the imported sheet? Unsaved changes on this sheet will be lost.",
    spellUseConfirmTitle: "Cast spell",
    spellUseConfirmMpShortfallBody: "You have {{haveMp}} MP but this spell costs {{cost}} MP. {{needHp}} HP will be spent to cover the remainder. Continue?",
    spellUseConfirmMpShortfallDangerBody: "You have {{haveMp}} MP but this spell costs {{cost}} MP. {{needHp}} HP will be spent to cover the remainder, bringing you to {{hpAfter}} HP (below 1 HP, including 0 or negative). Continue?",

    // Inventory
    equipped: "Equipped",
    consumables: "Consumables",
    others: "Others",
    weapons: "Weapons",
    armor: "Armor",
    bags: "Bags",
    itemName: "Item name",
    itemType: "Type",
    itemDescription: "Description",
    numberOwned: "Qty",
    weaponSlots: "Weapon slots",
    defense: "Defence",
    physicalDefense: "Physical Defence",
    magicalDefense: "Magical Defence",
    equippableSlots: "Equippable slots",
    slotWeapon1: "Weapon 1",
    slotWeapon2: "Weapon 2",
    slotWeapon3: "Weapon 3",
    slotHat: "Hat",
    slotFace: "Face",
    slotNecklace: "Necklace",
    slotPendant1: "Pendant 1",
    slotPendant2: "Pendant 2",
    slotPendant3: "Pendant 3",
    slotTorso: "Torso",
    slotRightShoulder: "Right shoulder",
    slotLeftShoulder: "Left shoulder",
    slotLeftArm: "Left arm",
    slotRightArm: "Right arm",
    slotLeftWrist: "Left wrist",
    slotRightWrist: "Right wrist",
    slotBelt: "Belt",
    slotLeftLeg: "Left leg",
    slotRightLeg: "Right leg",
    slotLeftAnkle: "Left ankle",
    slotRightAnkle: "Right ankle",
    slotLeftFoot: "Left foot",
    slotRightFoot: "Right foot",
    slotOther: "Other",
    fingers: "Fingers",
    slotLeftThumb: "L. thumb",
    slotLeftIndex: "L. index",
    slotLeftMiddle: "L. middle",
    slotLeftRing: "L. ring",
    slotLeftPinky: "L. pinky",
    slotRightThumb: "R. thumb",
    slotRightIndex: "R. index",
    slotRightMiddle: "R. middle",
    slotRightRing: "R. ring",
    slotRightPinky: "R. pinky",
    leftThumb: "L. thumb",
    leftIndex: "L. index",
    leftMiddle: "L. middle",
    leftRing: "L. ring",
    leftPinky: "L. pinky",
    rightThumb: "R. thumb",
    rightIndex: "R. index",
    rightMiddle: "R. middle",
    rightRing: "R. ring",
    rightPinky: "R. pinky",

    // Chat
    chatPlaceholder: "Message or /str +5, /pdmg 2d6...",
    chatWritePlaceholder: "Write here...",
    scrollUp: "Scroll up",
    scrollDown: "Scroll down",
    send: "Send",
    edit: "Edit",
    done: "Done",
    bold: "Bold",
    italic: "Italic",
    underline: "Underline",
    separator: "Separator",
    rolled: "Rolled",
    rerolled: "Rerolled",
    usingAFavor: "using a favor",
    criticalSuccess: "Critical success",
    criticalFailure: "Critical failure",
    applyMana: "Apply mana",
    mana: "Mana",

    // Notes
    notesPlaceholder: "Notes... Type inline rolls like [str +5] or [pdmg 2d6]...",
    notesEditor: "Editor",
    notesPreview: "Preview (rolls)",
    notesEmpty: "These are notes made on this sheet.",
    formatting: "Formatting",

    spellElement: "Element",

    // Settings
    uiColors: "UI colors",
    sheetPermissions: "Sheet's Permissions",
    canSee: "Can see",
    canEdit: "Can edit",
    color1: "Background",
    color3: "Border",
    color4: "Text",
    color5: "Accent",
    exportSheet: "Export sheet",
    importSheet: "Import sheet",
    exportEverything: "Export everything",
    importEverything: "Import everything",
    importFile: "Import from file",
    resetColorsTooltip: "Reset colors",

    /** Roll prep modal & quick-roll */
    rollQuickRollTooltip: "Shift+click : Quick Roll",
    rollQuickRollTooltipInverted: "Shift+click : Normal Roll",
    autoQuickRoll: "Auto quick roll",
    autoQuickRollOn: "On",
    autoQuickRollOff: "Off",
    isElementalToggle: "Is an elemental",
    rollPrepTitle: "Roll",
    rollPrepNoTalents: "No talents on this sheet.",
    rollPrepModifiers: "Modifiers",
    rollPrepExtraPlaceholder: "Manual modifier",
    rollPrepFinalFormula: "Final formula",
    rollPrepInvalidFormula: "That formula is not valid. Check your modifiers.",

    /** "Enter {field}..." — {field} is the lowercased label from t(key). */
    enterFieldTemplate: "Enter {field}...",
    statRollModifierPlaceholder: "+5 or -3",

    // Common
    add: "Add",
    remove: "Remove",
    save: "Save",
    cancel: "Cancel",
    close: "Close",
    confirm: "Confirm",
    ok: "OK",
    transfer: "Transfer",
    recipient: "Recipient",
    send: "Send",
    simplify: "Simplify",
    confirmSending: "Confirm sending",
    gave: "gave",
    to: "to",
    notEnoughMoney: "Not enough money",
    notEnoughToTransfer: "Not enough to transfer",
    unequipped: "Unequipped",
    slots: "Slots",
    item: "Item",
    itemName: "Item",
    itemDescription: "Description",
    selectItemToRemove: "Select item to remove",
    equipmentSlots: "Equipment Slots",
    currency: "Currency",
    goldCoin: "Gold Coin",
    silverCoin: "Silver Coin",
    copperCoin: "Copper Coin",
  },
  fr: {
    appTitle: "Foxyverse TTRPG",
    appSubtitle: "Une intégration Owlbear.Rodeo du système Foxyverse",

    // Sélection de fiche & permissions
    selectSheet: "Choisir la fiche",
    newSheet: "Nouvelle fiche",
    noSheet: "Aucune fiche",
    noAvailableSheet: "Aucune fiche disponible",
    settings: "Paramètres",
    permissions: "Permissions",
    view: "Voir",
    edit: "Modifier",
    linkToken: "Lier au token",
    gmOnly: "MJ uniquement",

    // Onglets
    tabBio: "Bio",
    tabStats: "Statistiques",
    tabSpells: "Sorts",
    tabInventory: "Inventaire",
    tabChat: "Chat",
    tabNotes: "Notes",
    tabSettings: "Paramètres",

    // Bio
    name: "Prénom",
    surname: "Nom",
    element: "Élément",
    class: "Classe",
    level: "Niveau",

    // Stats
    health: "Santé",
    mana: "Mana",
    favor: "Faveur",
    labelMaximum: "Max",
    labelCurrent: "Actuel",
    labelTemporary: "Temporaire",
    statBonusShort: "Bonus",
    defensesBlock: "Défenses",
    labelPhysicalShort: "Physique",
    labelMagicalShort: "Magique",
    defenseInfiniteDisplay: "∞",
    graphTitle: "Graphique",
    talentsBlock: "Talents",
    talentDefault: "Talent",
    talentDescPlaceholder: "écrivez la description du talent ici",
    talentDescTooltip: "Description du talent",
    itemTalent: "Talent d'objet",
    none: "—",
    totalPointsLabel: "Points Totaux",
    ptsSuffix: " pts",
    tempHP: "PV temp",
    currentHP: "PV actuels",
    maxHP: "PV max",
    currentMP: "PM actuels",
    maxMP: "PM max",
    currentFavor: "Faveur actuelle",
    maxFavor: "Faveur max",
    action: "Action",
    actionModifier: "Mod. action",
    speed: "Vitesse",
    speedModifier: "Mod. vitesse",
    baseStat: "Base",
    xpBonus: "Bonus XP",
    itemBonus: "Objet",
    passiveBonus: "Passif",
    total: "Total",
    roll: "dé",
    modifier: "Modificateur",
    quickMods: "Mod. rapides",
    knowledge: "Connaissances",
    addKnowledge: "Ajouter connaissance",
    tier: "Niveau",
    tier1: "Niveau 1 (-1)",
    tier2: "Niveau 2 (-3)",
    tier3: "Niveau 3 (-5)",
    tier4: "Niveau 4 (-10)",
    constitution: "Constitution",
    strength: "Force",
    intelligence: "Intelligence",
    perception: "Perception",
    social: "Social",
    agility: "Agilité",
    focus: "Focus",

    statAbbr_constitution: "con",
    statAbbr_strength: "for",
    statAbbr_intelligence: "int",
    statAbbr_perception: "per",
    statAbbr_social: "soc",
    statAbbr_agility: "agi",
    statAbbr_focus: "foc",

    inlineRoll_pdmg: "dgtp",
    inlineRoll_mdmg: "dgtm",
    inlineRoll_tdmg: "dgtb",
    inlineRoll_heal: "soin",
    inlineRoll_theal: "soint",
    inlineRoll_mana: "mana",
    inlineRoll_roll: "r",

    // Jets de dés
    statRoll: "Jet de stat",
    nat1: "Crit 1",
    nat20: "Crit 20",
    success: "Réussi",
    failure: "Échec",
    dc: "DD",
    result: "Résultat",
    rollModalRerollTitle: "{ordinal} relancement — résultat",
    reroll: "Relancer",
    rerollCost: "Coûte 1 Faveur",
    rollClampFloorNote: "relevé au minimum",
    rollClampCeilNote: "plafonné au maximum",
    rollSpeed: "Lancer Vitesse",
    linkTokenToSheet: "Lier le token sélectionné",
    linkedTokens: "Tokens liés",
    unlink: "Délier",
    noTokenSelected: "Sélectionnez un token sur la carte d'abord",
    physicalDamage: "Dégâts physiques",
    magicDamage: "Dégâts magiques",
    trueDamage: "Dégât Brut",
    heal: "Soin",
    overHeal: "Soin temporaire (PV temp)",
    otherRoll: "Autre jet",
    applyDamage: "Appliquer dégâts",
    applyPhysicalDamages: "Appliquer dégâts physiques",
    applyMagicalDamages: "Appliquer dégâts magiques",
    applyTrueDamages: "Appliquer dégâts bruts",
    applyHeal: "Appliquer soin",
    applyOverHeal: "Appliquer PV temp",
    healedFor: "s'est soigné de",
    tempHealedFor: "a gagné",
    tookDamage: "a subi",
    regainedMana: "a récupéré",
    manaPoints: "points de mana",
    chatApplyPhysicalDamage: "{{name}} a pris {{actual}} dégâts physiques [{{raw}} - {{def}} PD]",
    chatApplyPhysicalDamageElemental: "{{name}} a bloqué les dégâts physiques (élémentaire) [jet {{raw}}]",
    chatApplyMagicalDamage: "{{name}} a pris {{actual}} dégâts magiques [{{raw}} - {{def}} MD]",
    chatApplyMagicalDamageToMana: "{{name}} a perdu {{actual}} points de mana (dégâts magiques) [2×{{raw}} − {{def}} MD]",
    chatApplyMagicalDamageElemental: "{{name}} a pris {{total}} dégâts magiques (élémentaire) [2×{{raw}} − {{def}} MD] ; mana perdue : {{manaLost}} (après PV temporaires).",
    chatApplyTrueDamage: "{{name}} a pris {{actual}} dégâts bruts",
    chatApplyTrueDamageToMana: "{{name}} a perdu {{actual}} points de mana (dégâts bruts)",
    chatApplyTrueDamageElemental: "{{name}} a pris {{total}} dégâts bruts (élémentaire) ; mana perdue : {{manaLost}} (après PV temporaires).",
    chatCurrencyGift: "{{fromName}} {{gave}} {{gold}} {{goldLabel}}, {{silver}} {{silverLabel}}, {{copper}} {{copperLabel}} {{to}} {{toName}}",
    chatItemGift: "{{fromName}} {{gave}} {{qty}} × {{itemName}} {{to}} {{toName}}",
    gainedVerb: "a gagné",
    temporaryHitPointsPhrase: "points de vie temporaires",
    alreadyAtFullHealth: "avait déjà tous ses points de vie",
    nowAtFullHealth: "a maintenant tous ses points de vie",
    alreadyAtFullMana: "avait déjà tous ses points de mana",
    nowAtFullMana: "a maintenant tous ses points de mana",

    // Sorts
    spellName: "Nom du sort",
    spellEffect: "Effet",
    spellCost: "Coût",
    costHP: "PV",
    costMP: "PM",
    use: "Jeter",
    used: "Utilisé",
    times: "fois",
    continuous: "Continu",
    spellArmed: "Armé",
    cost: "coût",
    toggle: "Ouvrir/fermer",
    reorder: "Réordonner",
    selectSpellToRemove: "Sélectionnez un sort à supprimer",
    deductCost: "Déduire coût",
    notEnoughMP: "Pas assez de PM — utiliser les PV ?",
    confirmUseHP: "Utiliser les PV à la place des PM va vous blesser. Continuer ?",
    confirmCastBelowZeroHP: "Lancer ce sort vous laisserait à 0 PV ou en dessous. Voulez-vous continuer ?",

    // Stats (template UI)
    talentName: "Nom du talent",
    detailedStatistics: "Statistiques détaillées",
    overwriteBonusesHere: "Écrasez les bonus ici",
    points: "points",
    save: "Sauvegarder",
    confirmDelete: "Êtes-vous sûr de vouloir supprimer ?",
    confirmModalTitle: "Confirmation",
    confirmDeleteSheetBody: "Supprimer la fiche « {{name}} » ? Cette action est irréversible.",
    confirmImportOverwriteBody: "Remplacer « {{name}} » par la fiche importée ? Les modifications non enregistrées seront perdues.",
    spellUseConfirmTitle: "Lancer le sort",
    spellUseConfirmMpShortfallBody: "Vous avez {{haveMp}} PM mais ce sort coûte {{cost}} PM. {{needHp}} PV seront dépensés pour le reste. Continuer ?",
    spellUseConfirmMpShortfallDangerBody: "Vous avez {{haveMp}} PM mais ce sort coûte {{cost}} PM. {{needHp}} PV seront dépensés pour le reste, ce qui vous mènerait à {{hpAfter}} PV (moins de 1 PV, y compris 0 ou négatif). Continuer ?",

    // Inventaire
    equipped: "Équipé",
    consumables: "Consommables",
    others: "Autres",
    weapons: "Armes",
    armor: "Armure",
    bags: "Sacs",
    itemName: "Nom",
    itemType: "Type",
    itemDescription: "Description",
    numberOwned: "Qté",
    weaponSlots: "Emplacements arme",
    defense: "Défense",
    physicalDefense: "Défense Physique",
    magicalDefense: "Défense Magique",
    equippableSlots: "Emplacements équipables",
    slotWeapon1: "Arme 1",
    slotWeapon2: "Arme 2",
    slotWeapon3: "Arme 3",
    slotHat: "Chapeau",
    slotFace: "Visage",
    slotNecklace: "Collier",
    slotPendant1: "Pendentif 1",
    slotPendant2: "Pendentif 2",
    slotPendant3: "Pendentif 3",
    slotTorso: "Torse",
    slotRightShoulder: "Épaule droite",
    slotLeftShoulder: "Épaule gauche",
    slotLeftArm: "Bras gauche",
    slotRightArm: "Bras droit",
    slotLeftWrist: "Poignet gauche",
    slotRightWrist: "Poignet droit",
    slotBelt: "Ceinture",
    slotLeftLeg: "Jambe gauche",
    slotRightLeg: "Jambe droite",
    slotLeftAnkle: "Cheville gauche",
    slotRightAnkle: "Cheville droite",
    slotLeftFoot: "Pied gauche",
    slotRightFoot: "Pied droit",
    slotOther: "Autre",
    fingers: "Doigts",
    slotLeftThumb: "Pouce G",
    slotLeftIndex: "Index G",
    slotLeftMiddle: "Majeur G",
    slotLeftRing: "Annulaire G",
    slotLeftPinky: "Auriculaire G",
    slotRightThumb: "Pouce D",
    slotRightIndex: "Index D",
    slotRightMiddle: "Majeur D",
    slotRightRing: "Annulaire D",
    slotRightPinky: "Auriculaire D",
    leftThumb: "Pouce G",
    leftIndex: "Index G",
    leftMiddle: "Majeur G",
    leftRing: "Annulaire G",
    leftPinky: "Auriculaire G",
    rightThumb: "Pouce D",
    rightIndex: "Index D",
    rightMiddle: "Majeur D",
    rightRing: "Annulaire D",
    rightPinky: "Auriculaire D",

    // Chat
    chatPlaceholder: "Message ou /str +5, /pdmg 2d6...",
    chatWritePlaceholder: "Écrire ici...",
    scrollUp: "Défiler vers le haut",
    scrollDown: "Défiler vers le bas",
    send: "Envoyer",
    edit: "Éditer",
    done: "Terminer",
    bold: "Gras",
    italic: "Italique",
    underline: "Souligner",
    separator: "Séparateur",
    rolled: "Jet de",
    rerolled: "Relancé",
    usingAFavor: "en utilisant une faveur",
    criticalSuccess: "Succès critique",
    criticalFailure: "Échec critique",
    applyMana: "Appliquer mana",
    mana: "Mana",

    // Notes
    notesPlaceholder: "Notes... Jets inline comme [str +5] ou [pdmg 2d6]...",
    notesEditor: "Éditeur",
    notesPreview: "Aperçu (jets)",
    notesEmpty: "Ce sont des notes prises sur cette fiche.",
    formatting: "Mise en forme",

    spellElement: "Élément",

    // Paramètres
    uiColors: "Couleurs de l'IU",
    sheetPermissions: "Permissions de la fiche",
    canSee: "Peut voir",
    canEdit: "Peut modifier",
    color1: "Fond",
    color3: "Bordure",
    color4: "Texte",
    color5: "Accent",
    exportSheet: "Exporter la fiche",
    importSheet: "Importer la fiche",
    exportEverything: "Tout exporter",
    importEverything: "Tout importer",
    importFile: "Importer un fichier",
    resetColorsTooltip: "Réinitialiser les couleurs",

    /** Jet avec talents */
    rollQuickRollTooltip: "Maj+clic : jet rapide",
    rollQuickRollTooltipInverted: "Maj+clic : jet normal",
    autoQuickRoll: "Jet rapide auto",
    autoQuickRollOn: "Activé",
    autoQuickRollOff: "Désactivé",
    isElementalToggle: "Est un élémentaire",
    rollPrepTitle: "Jet",
    rollPrepNoTalents: "Aucun talent sur cette fiche.",
    rollPrepModifiers: "Modificateurs",
    rollPrepExtraPlaceholder: "Modificateurs manuels",
    rollPrepFinalFormula: "Formule finale",
    rollPrepInvalidFormula: "Formule invalide. Vérifiez vos modificateurs.",

    /** "Enter {field}..." — {field} is the lowercased label from t(key). */
    enterFieldTemplate: "Saisir {field}...",
    statRollModifierPlaceholder: "+5 ou -3",

    // Commun
    add: "Ajouter",
    remove: "Retirer",
    save: "Enregistrer",
    cancel: "Annuler",
    close: "Fermer",
    confirm: "Confirmer",
    ok: "OK",
    transfer: "Transférer",
    recipient: "Destinataire",
    send: "Envoyer",
    simplify: "Simplifier",
    confirmSending: "Confirmer l'envoi de",
    gave: "a donné",
    to: "à",
    notEnoughMoney: "Pas assez d'argent",
    notEnoughToTransfer: "Pas assez pour transférer",
    unequipped: "Non équipé",
    slots: "Slots",
    item: "Objet",
    itemName: "Objet",
    itemDescription: "Description",
    selectItemToRemove: "Choisir l'objet à retirer",
    equipmentSlots: "Slots d'Équipement",
    currency: "Monnaie",
    goldCoin: "Pièce d'Or",
    silverCoin: "Pièce d'Argent",
    copperCoin: "Pièce de Cuivre",
  },
};

let currentLocale = "fr";

export function setLocale(locale) {
  if (translations[locale]) currentLocale = locale;
  return currentLocale;
}

export function getLocale() {
  return currentLocale;
}

export function t(key) {
  return translations[currentLocale]?.[key] ?? translations.en[key] ?? key;
}

/** Placeholder like "Enter name..." / "Saisir prénom..." — pass a translation key for the field label (e.g. "name", "tempHP"). */
export function enterField(labelKey) {
  const raw = t(labelKey);
  const field = String(raw).toLowerCase();
  return t("enterFieldTemplate").replace("{field}", field);
}
